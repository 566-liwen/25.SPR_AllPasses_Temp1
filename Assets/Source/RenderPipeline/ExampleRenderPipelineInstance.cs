...
public class RenderPipelineInstance : RenderPipeline
{
    ...
    protected void OnPostCameraCulling(ScriptableRenderContext context, Camera camera, ref CullingResults cullingResults)
    {
        ...
        Vector3 targetDir = lightTrans.position - cameraTrans.position;
        float angle = Vector3.Angle(targetDir, cameraTrans.forward);
        //Debug.Log(angle);
        Vector3 cameraPostion = cameraTrans.position;
        Shader.SetGlobalColor("_CameraVector", new Color(cameraPostion.x, cameraPostion.y, cameraPostion.z, 1));
        Shader.SetGlobalFloat("_AngleCameraLight", angle);
